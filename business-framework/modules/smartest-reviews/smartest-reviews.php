...
*/
		...
		add_action( 'admin_init', array(&$this, 'smartestt_create_reviews_page'));//@note, but for stand-alone plugin hook to after_setup_theme
    ...
    function addmenu() {
		...
		if(get_option('smartestb_add_reviews') == 'true') {       
			add_menu_page(__('Smartest Reviews', 'smartestb'), __('Smartest Reviews', 'smartestb'), 'edit_others_posts', 'smar_view_reviews', array(&$this, 'admin_view_reviews'), $this->getpluginurl() . 'star.png', 62); /* was 50 */
		}
   }
...
function smartestt_create_reviews_page() {
	...
	if(get_option('smartestb_add_reviews') == 'true') {
		smartestbusiness_insert_post('page', esc_sql( _x('reviews', 'page_slug', 'smartestb') ), 'smartest_reviews_page_id', __('Reviews', 'smartestb'), '[SMAR_INSERT]' );
	}
